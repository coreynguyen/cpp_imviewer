...
bool is_utf8 (const char* str) {
    ...
    int num;
    while (*bytes != 0x00) {
        if ((*bytes & 0x80) == 0x00) {num = 1;} // U+0000 to U+007F
        else if ((*bytes & 0xE0) == 0xC0) {num = 2;} // U+0080 to U+07FF
        else if ((*bytes & 0xF0) == 0xE0) {num = 3;} // U+0800 to U+FFFF
        else if ((*bytes & 0xF8) == 0xF0) {num = 4;} // U+10000 to U+10FFFF
        else {return false;}
        bytes += 1;
        for (int i = 1; i < num; ++i) {
...
bool is_valid_utf8(const char* str) {
    ...
    int num;
    while (*bytes != 0x00) {
        if ((*bytes & 0x80) == 0x00) {cp = (*bytes & 0x7F); num = 1;}       // U+0000 to U+007F
        else if ((*bytes & 0xE0) == 0xC0) {cp = (*bytes & 0x1F); num = 2;}  // U+0080 to U+07FF
        else if ((*bytes & 0xF0) == 0xE0) {cp = (*bytes & 0x0F); num = 3;}  // U+0800 to U+FFFF
        else if ((*bytes & 0xF8) == 0xF0) {cp = (*bytes & 0x07); num = 4;}  // U+10000 to U+10FFFF
        else {return false;}
        bytes += 1;
        for (int i = 1; i < num; ++i) {
            if ((*bytes & 0xC0) != 0x80) {return false;}
