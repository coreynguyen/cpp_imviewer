...
std::string trim(const std::string& str, const std::string& whitespace) {
    ...
    /*
    unsigned int len = whitespace.size();
    size_t strBegin = 0, strEnd = len, a, b;
    for (unsigned int i = 0; i < len; i++) {
        a = str.find_first_not_of(whitespace.substr(i, 1));
        if (a == std::string::npos) {continue;}
        if (a > strBegin) {strBegin = a;}
        b = str.find_last_not_of(whitespace.substr(i, 1));
        if (b < strEnd) {strEnd = b;}
        }
...
std::string get_part_date(const std::string &datepart, time_t now) {
    ...
    */
    ...
    strftime(buffer, sizeof(buffer), datepart.c_str(), &timeinfo);
...
std::string wstring_to_string (std::wstring ws) {
    ...
    // above not working well with MSCV compiler for some reason
    return unicode_to_utf8(ws);
	}
...
int findString (std::string &searchingIn, std::string findThis) {
    ...
    if (searchingIn.size() > 0) {
        found = (size_t)searchingIn.find(findThis);
        if (found > searchingIn.size()) {result = -1;} else {result = (signed int)found;}
        }
    return result;
    }
