/*	======================================================================
	...
	Change Log:
	[2023-04-14]
        > added XOR function for encrpyting data
	[2023-03-18]
        > added Md5, requires stringext.h for int to hex function
	[2022-07-13]
		> added fcs16
	[2022-04-02]
...
#include <iostream>
#include <string>
#include <cmath>		// needed for pow() ceil()
...
class bytestream {
  public:
	...
	void copy (const char* src_buf, size_t src_len, size_t src_pos = 0, size_t tar_dest = 0);
	size_t tell ();
	...
	std::string md5 (size_t crc_pos = 0, size_t crc_len = 0);
	std::string base64_encode (size_t crc_pos = 0, size_t crc_len = 0);
	void base64_decode (std::string &input, size_t tar_pos = 0);
	void apply_xor_key (const char* key, size_t keylen, size_t tar_pos = 0, size_t tar_size = 0);
    void asPNG (char* &buf, unsigned int width, unsigned int height, unsigned char channels, bool flipV = false, bool reverse = false);
	uint8_t readUbyte ();
	uint16_t readUshort ();
	uint32_t readUlong ();
