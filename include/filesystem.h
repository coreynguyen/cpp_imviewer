...
#define SHFILEOPSTRUCTW _SHFILEOPSTRUCTW
...
namespace os {
    ...
    /**
     * Use launchShellEx to open associated programs in windows, like notepad
     * @param <aPath> path to file
     * @param <aParams> any params
     * @param <holdHost> pauses parent app until the lauched program is closed
     * @return Int, error code
     */
    int start (std::wstring aPath, std::wstring aParams = L"", bool holdHost = false);

    /**
     * Opens directory if not already open
     * @param path
     * @return None
     */
    void openPathW(std::wstring path);
     ...
     */
    std::string getSaveFileName(const char* filter = "All Files (*.*)\0*.*\0", HWND owner = NULL);
     ...
     * @return Returns true on success, false on failure
     */
    bool deleteFile (std::string file, bool permanent = false);
     ...
     * @return Returns true on success, false on failure
     */
    bool deleteFileW (std::wstring file, bool permanent = false);
   ...
   /**
    ...
    /**
     * \brief gets the name of the PC
     * \return [Std::String] name of PC
     */
